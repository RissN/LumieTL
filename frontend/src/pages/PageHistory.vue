  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-[#8A8A96]">Catatan terjemahan sebelumnya</p>
      </div>
      <div class="flex gap-2">
        ...
        <button
          @click="clearAll"
          class="px-3 py-1.5 text-xs bg-[#222228] hover:bg-[#2A2A30] text-[#E05C5C] rounded border border-[#2A2A30]"
        >
          Hapus Semua
        </button>
      </div>
      ...
      <table class="w-full text-left text-xs">
        ...
        <tbody class="divide-y divide-[#2A2A30]">
          <tr v-for="item in items" :key="item.id" class="hover:bg-[#222228]/50">
            ...
            <td class="p-3 text-[#8A8A96]">{{ item.source_lang }} ➔ {{ item.target_lang }}</td>
            <td class="p-3 text-[#E8E8ED]">{{ item.engine }}</td>
            <td class="p-3 text-[#8A8A96]">{{ item.duration ? item.duration.toFixed(2) + 's' : '—' }}</td>
            <td class="p-3">
              <span v-if="item.success" class="text-[#4CAF82]">✅ Selesai</span>
              <span v-else class="text-[#E05C5C]">❌ Gagal</span>
            </td>
            <td class="p-3">
              <button
                @click="deleteItem(item.id)"
                class="text-xs text-[#E05C5C] hover:underline"
              >
                Hapus
              </button>
            </td>
